...
describe('FlexBox', () => {
    ...
    it('should display content in 100% in mobile', () => {
        expect(
            compiled
                .querySelector('.aof-footer > div:first-child')
                .getAttribute('fxFlex')
        ).toEqual('')
    })
    ...
    it('should restrain content to correct width on web', () => {
        expect(
            compiled
                .querySelector('.aof-footer > div:first-child')
                .getAttribute('fxFlex.gt-sm')
        ).toEqual('960px')
    })

    it('should center content', () => {
        expect(
            compiled.querySelector('.aof-footer').getAttribute('fxlayoutalign')
        ).toEqual('center center')
        expect(
            compiled.querySelector('.aof-footer').getAttribute('fxlayout')
        ).toEqual('row')
    })

    it('should contain a linked-in link', () => {
        expect(compiled.querySelector('.aof-linkedin-action')).toBeTruthy()
        expect(compiled.querySelector('.aof-linkedin-action').href).toEqual(
            'https://www.linkedin.com/in/jmilanes/'
        )
    })

    it('should contain a github link', () => {
        expect(compiled.querySelector('.aof-github-action')).toBeTruthy()
        expect(compiled.querySelector('.aof-github-action').href).toEqual(
